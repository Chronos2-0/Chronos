...
// declare win variable
let win;

// declaring a createWindow function
function createWindow() {
  // assign win to an instance of a new browser window.
  win = new BrowserWindow({
    // giving our window its width
    width: 900,
    // giving our window its height
    height: 800,
    // specify the path of the icon
    icon: path.join(__dirname, 'app/assets/icons/AppIcon.icns'),
    // enable node inegreation --> node intgeration, default is usally false
    webPreferences: {
  ...
  // Development
  // loads our application window to localHost 8080, application will not render without this loadUrl
...
// invoke createWindow function on Electron application load 
...
app.on('window-all-closed', () => {
  ...
  // process platform is a property that return a string identifying the OS platform on which NodeJs process is running
...
ipcMain.on('setup', (message) => {
  ...
  // destructure setupRequired from state constant 
  const { setupRequired } = state;
  // assigning message object a property of return value and assigning it the setupRequired from state destructuring 
...
ipcMain.on('dashboard', (message) => {
  ...
  // destructure services from state... what is services? 
  ...
  if (databaseType === 'SQL') {
    ...
    pool.query(getCommunications, (err, result) => {
      if (err) {
        ...
        // after requiring dialog in the topmost section of main. We invoke the method showMessagebox passing the error object we created 
